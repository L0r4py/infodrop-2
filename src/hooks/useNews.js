...
export const useNews = () => {
    ...
    const fetchGlobalStats = useCallback(async () => {
        ...
        try {
            ...
            const [articlesRes, sourcesRes] = await Promise.all([
                // Compte les articles récents
                supabase.from('articles').select('*', { count: 'exact', head: true }).gte('pubDate', twentyFourHoursAgo),

                // ✅ CORRECTION : Compte les sources actives directement depuis la table 'sources'
            ...
            setGlobalStats({
                ...
                articles_publies_24h: articlesRes.count || 0
            });
        } catch (err) {
    ...
    const markAsRead = useCallback(async (id) => {
        ...
        setNews(prevNews =>
            prevNews.map(item =>
                item.id === id ? { ...item, views: (Number(item.views) || 0) + 1 } : item
            )
        );
        ...
        if (USE_SUPABASE && isSupabaseConfigured() && supabase) {
            try {
                ...
                console.warn("Erreur 'increment_views':", err.message); // On ne bloque pas l'UI
            }
        }
