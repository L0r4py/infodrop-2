...
// Fonction pour obtenir les sources par orientation
export const getSourcesByOrientation = (orientation) => {
    return newsSources.filter(source => source.orientation === orientation);
};

// Fonction pour obtenir les sources par catégorie
export const getSourcesByCategory = (category) => {
    return newsSources.filter(source => source.category === category);
};

// Fonction pour compter les sources par orientation
export const countSourcesByOrientation = () => {
    const counts = {};
    orientationOrder.forEach(orientation => {
        counts[orientation] = newsSources.filter(source => source.orientation === orientation).length;
    });
    return counts;
};

// Fonction pour obtenir une source par son nom
export const getSourceByName = (name) => {
    return newsSources.find(source => source.name === name);
};

// Fonction pour obtenir toutes les URLs RSS
...
export default {
    ...
    getOrientationColor,
    getOrientationLabel,
    getSourcesByOrientation,
    getSourcesByCategory,
    countSourcesByOrientation,
    getSourceByName,
    getRSSFeeds
};